...
public sealed class UpdateIncomeHandler : IRequestHandler<UpdateIncomeCommand, Unit>
{
    private readonly IBudgetRepository _repo; private readonly IBudgetEventPublisher _pub;
    public UpdateIncomeHandler(IBudgetRepository repo, IBudgetEventPublisher pub) { _repo = repo; _pub = pub; }
    public async Task<Unit> Handle(UpdateIncomeCommand cmd, CancellationToken ct)
    {
        ...
        budget.UpdateIncome(cmd.IncomeId, cmd.Name, cmd.Amount);
