...
public sealed class RemoveExpenseHandler : IRequestHandler<RemoveExpenseCommand, Unit>
{
    private readonly IBudgetRepository _repo; private readonly IBudgetEventPublisher _pub;
    public RemoveExpenseHandler(IBudgetRepository repo, IBudgetEventPublisher pub) { _repo = repo; _pub = pub; }
    public async Task<Unit> Handle(RemoveExpenseCommand cmd, CancellationToken ct)
    {
        ...
        budget.RemoveExpense(cmd.ExpenseId);
